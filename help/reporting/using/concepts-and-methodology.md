...
1. Använda manuellt definierade värdeintervall. Exempel: ålder, genomsnittlig kundvagn, antal öppnade leveranser osv.). Mer information finns i [Definiera varje bin](#defining-each-bin).
...
Klicka på knappen **[!UICONTROL Add]** för att skapa en ny behållare och visa de värden som ska grupperas i behållaren.
...
Du kan använda en SQL-mask för att kombinera flera värden till ett filter. Det gör du genom att markera **[!UICONTROL Yes]** i kolumnen **[!UICONTROL Use an SQL mask]** och ange det SQL-filter som ska användas i kolumnen **[!UICONTROL Value or expression]**.

I exemplet nedan är alla e-postdomäner som börjar med **yahoo** (yahoo.fr, yahoo.com, yahoo.be, osv.) eller med **ymail** (ymail.com, ymail.eu, osv.) grupperas under etiketten **YAHOO!**, samt adresser med  **rocketmail.** comdomain.
...
Det går också att generera behållare automatiskt. Det gör du genom att klicka på länken **[!UICONTROL Generate bins...]**.
...
Om du vill skapa en rapport med den här uppräkningen skapar du en kub med dimensionen **[!UICONTROL Email domain]**. Välj alternativet **[!UICONTROL Enable binning]** och sedan **[!UICONTROL Dynamically link the values to an enumeration]**. Välj sedan uppräkningen **Domäner** enligt ovan. Alla värden som saknar angivet alias grupperas om under etiketten **Övrigt**.

![](assets/nmx_add_dimension.png)

Skapa sedan en rapport baserad på den här kuben för att visa värdena.

Du behöver bara ändra uppräkningen för att uppdatera den relaterade rapporten. Skapa till exempel **Adobe**-värdet och lägg till aliaset **adobe.com** så uppdateras rapporten automatiskt med Adobe-värdet på uppräkningsnivån.
...
Måtttyperna definieras på fliken **[!UICONTROL Measures]** i kuben. Du kan beräkna summor, medelvärden, avvikelser osv.
...
1. Med länken **[!UICONTROL Filter the measure data...]** kan du begränsa beräkningsfältet och bara använda det på specifika data i databasen.
...
Ange kategorin som rapporten tillhör samt dess relevans. Mer information finns i [den här sidan](../../reporting/using/configuring-access-to-the-report.md#report-display-context) i **visningssekvensen** och **Definiera filteralternativen**-avsnitten.
