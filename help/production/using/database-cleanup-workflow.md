...
Med arbetsflödet **[!UICONTROL Database cleanup]** som är tillgängligt via noden **[!UICONTROL Administration > Production > Technical workflows]** kan du ta bort föråldrade data för att undvika exponentiell tillväxt i databasen. Arbetsflödet utlöses automatiskt utan användaråtgärder.

![rensa](assets/ncs_cleanup_workflow.png)
...
Fälten i fönstret **[!UICONTROL Purge of data]** sammanfaller med följande alternativ. Dessa används av vissa av de åtgärder som körs i arbetsflödet **[!UICONTROL Database cleanup]**:
...
* Besökarprofiler: **NmsCleanup_VisitorPurgeDelay** (se [Rensning av besökare](#cleanup-of-visitors))
...
Den första åtgärden som utförs av arbetsflödet **[!UICONTROL Database cleanup]** tar bort alla grupper med **deleteStatus != 0**-attribut från **NmsGroup**. Poster som är länkade till de här grupperna och som finns i andra tabeller tas också bort.
...
1. Alla **Select**-typlistor som återställs av åtgärden tas bort med följande fråga:
...
Arbetsflödet **[!UICONTROL Database cleanup]** tar också bort leveranser på mellankällservern(erna).
...
1. Arbetsflödet **[!UICONTROL Database cleanup]** skapar en lista över leveranser som har upphört att gälla. Den här listan innehåller alla utgångna leveranser med en annan status än **[!UICONTROL Finished]** samt nyligen stoppade leveranser med över 10 000 obearbetade meddelanden. Följande fråga används:
   ...
   där `delivery mode 1` matchar läget **[!UICONTROL Mass delivery]** matchar `state 51` läget **[!UICONTROL Start pending]**, `state 85` läget **[!UICONTROL Stopped]** och det högsta antalet massuppdateringar av leveransloggar på leveransservern är 10 000.
...
1. I listan över utgångna leveranser växlar leveransloggar med statusen **[!UICONTROL Pending]**, växlar till **[!UICONTROL Delivery cancelled]** och alla leveranser i listan till **[!UICONTROL Finished]**.
   ...
   där `$(curdate)` är det aktuella datumet för databasservern, är `$(bl)` identifieraren för leveransloggmeddelandet, `$(dl)` är leveransidentifieraren, `delivery status 6` matchar **[!UICONTROL Pending]**-statusen och `delivery status 7` matchar **[!UICONTROL Delivery cancelled]**-statusen.

   ```sql
   UPDATE NmsDelivery SET iState = 95, tsLastModified = $(curdate), tsBroadEnd = tsValidity WHERE iDeliveryId = $(dl)
   ```

   där `delivery state 95` matchar **[!UICONTROL Finished]**-statusen och `$(dl)` är leveransens identifierare.
...
1. Alla fragment (**deliveryParts**) av föråldrade leveranser tas bort och alla föråldrade fragment av pågående meddelandeleveranser tas bort. Massborttagning används för båda dessa uppgifter.
   ...
   där `delivery state 95` matchar statusen **[!UICONTROL Finished]** matchar `delivery state 85` statusen **[!UICONTROL Stopped]** och `$(curDate)` är det aktuella serverdatumet.
...
Den här uppgiften tar bort alla arbetstabeller från databasen som matchar leveranser vars status är **[!UICONTROL Being edited]**, **[!UICONTROL Stopped]** eller **[!UICONTROL Deleted]**.
...
1. Tabellerna som används av pågående arbetsflöden exkluderas sedan. För att göra detta återställs listan över pågående leveranser med följande fråga:
   ...
   där `0` är det värde som matchar **[!UICONTROL Being edited]** leveransstatus, matchar `85` **[!UICONTROL Stopped]**-status och `100` **[!UICONTROL Deleted]**-status.
...
1. Aktivitetshistoriktabeller av typen &quot;inkrementell fråga&quot; exkluderas med hjälp av följande frågor:
   ...
   där `$(strcondition)` är listan med tabeller som matchar masken **wkfhisto%**.
...
där `status 2` matchar statusen **[!UICONTROL Valid]** är `$(tsDate1)` det aktuella serverdatumet och `$(tsDate2)` matchar alternativet **NmsCleanup_LastCleanup**.
...
1. När du använder mellanleverantörer refereras inte tabellen **NmsBroadLogMid** i leveransmappningar. Schemat **nms:broadLogMid** läggs till i listan som återställdes av föregående fråga.
1. Arbetsflödet för **databasrensning** rensar sedan bort föråldrade data från tidigare återställda tabeller. Följande fråga används:
...
Om alternativet **XtkCleanup_NoStats** inte finns, eller om värdet är 0, kommer detta att köra lagringsoptimeringen i utförligt läge (VACUUM VERBOSE ANALYZE) på PostgreSQL och uppdatera statistik för alla andra databaser. Kontrollera PostgreSQL-loggarna för att se till att kommandot körs. VACUUM kommer att mata ut rader i formatet: `INFO: vacuuming "public.nmsactivecontact"` och ANALYZE kommer att mata ut rader i formatet: `INFO: analyzing "public.nmsactivecontact"`.
